  ...
  const PointCloud2 & cloud)
{
  ...
  const auto check_field = [](
    ...
    const decltype(sensor_msgs::PointField::datatype) datatype) -> bool8_t {
      bool8_t res = true;
  ...
  if (!check_field(cloud.fields[0U], "x", 0U, sensor_msgs::PointField::FLOAT32)) {
    throw std::runtime_error("PointCloud doesn't have correct x field");
  } else if (!check_field(cloud.fields[1U], "y", 4U, sensor_msgs::PointField::FLOAT32)) {
    throw std::runtime_error("PointCloud doesn't have correct y field");
  } else if (!check_field(cloud.fields[2U], "z", 8U, sensor_msgs::PointField::FLOAT32)) {
    throw std::runtime_error("PointCloud doesn't have correct z field");
  } else {
    // do nothing
  }
  if (cloud.fields.size() >= 4U) {
    if (!check_field(cloud.fields[3U], "intensity", 12U, sensor_msgs::PointField::FLOAT32)) {
      if (!check_field(cloud.fields[3U], "intensity", 16U, sensor_msgs::PointField::UINT8)) {
        ret = false;
      }
    }
  } else {
...
void PointCloudIts::reset(sensor_msgs::PointCloud2 & cloud, uint32_t idx)
{
  ...
  const std::size_t size)
{
  init_pcl_msg(
    ...
    "x", 1U, sensor_msgs::PointField::FLOAT32,
    "y", 1U, sensor_msgs::PointField::FLOAT32,
    "z", 1U, sensor_msgs::PointField::FLOAT32,
    "intensity", 1U, sensor_msgs::PointField::FLOAT32);
}
...
StaticTransformer::StaticTransformer(const geometry_msgs::Transform & tf)
{
  ...
  m_intensity_it_float32(msg, "intensity")
{
  auto && intensity_field_it =
    ...
    [](const sensor_msgs::PointField & field) {return field.name == "intensity";});
