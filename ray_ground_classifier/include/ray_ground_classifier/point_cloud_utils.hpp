...
#include <autoware_auto_msgs/PointClusters.h>
#include <ray_ground_classifier/types.hpp>
#include <ray_ground_classifier/float_comparisons.hpp>
#include <geometry_msgs/TransformStamped.h>
#include <ray_ground_classifier/common_3d.hpp>
...
public:
  ...
  void reset(sensor_msgs::PointCloud2 & cloud, uint32_t idx = 0);
...
std::size_t index_after_last_safe_byte_index(const sensor_msgs::PointCloud2 & msg) noexcept;
...
/// {memcpy(&pt, msg.data[idx], ret.point_step);}`
 SafeCloudIndices sanitize_point_cloud(const sensor_msgs::PointCloud2 & msg);
...
std::pair<autoware_auto_msgs::PointClusters::_points_type::iterator,
  autoware_auto_msgs::PointClusters::_points_type::iterator>
get_cluster(autoware_auto_msgs::PointClusters & clusters, const std::size_t cls_id);
...
std::pair<autoware_auto_msgs::PointClusters::_points_type::const_iterator,
  autoware_auto_msgs::PointClusters::_points_type::const_iterator>
get_cluster(const autoware_auto_msgs::PointClusters & clusters, const std::size_t cls_id);
...
/// the fourth field, otherwise return false
 bool8_t
has_intensity_and_throw_if_no_xyz(const PointCloud2::Ptr & cloud);
...
{
  static constexpr auto DATATYPE = sensor_msgs::PointField::INT8;
};

template<>
struct _create_custom_pcl_datatype<uint8_t>
{
  static constexpr auto DATATYPE = sensor_msgs::PointField::UINT8;
};

template<>
struct _create_custom_pcl_datatype<int16_t>
{
  static constexpr auto DATATYPE = sensor_msgs::PointField::INT16;
};

template<>
struct _create_custom_pcl_datatype<uint16_t>
{
  static constexpr auto DATATYPE = sensor_msgs::PointField::UINT16;
};

template<>
struct _create_custom_pcl_datatype<int32_t>
{
  static constexpr auto DATATYPE = sensor_msgs::PointField::INT32;
};

template<>
struct _create_custom_pcl_datatype<uint32_t>
{
  static constexpr auto DATATYPE = sensor_msgs::PointField::UINT32;
};

template<>
struct _create_custom_pcl_datatype<float32_t>
{
  static constexpr auto DATATYPE = sensor_msgs::PointField::FLOAT32;
};

template<>
struct _create_custom_pcl_datatype<float64_t>
{
  static constexpr auto DATATYPE = sensor_msgs::PointField::FLOAT64;
};

template<typename T>
sensor_msgs::PointCloud2::Ptr create_custom_pcl(
  const std::vector<std::string> & field_names,
  const uint32_t cloud_size)
{
  using sensor_msgs::PointCloud2;
  PointCloud2::Ptr msg = std::make_shared<PointCloud2>();
...
public:
  ...
  explicit StaticTransformer(const geometry_msgs::Transform & tf);
...
private:
  ...
  decltype(sensor_msgs::PointField::datatype) m_intensity_datatype;
